class SuperAdminHomePage {
  ...
  get editBtn() {
    return $(
      '.list_container__21Buw > table > tbody > tr:nth-child(1) > td:nth-child(5) > a > button'
    );
  }
  get deleteBtn() {
    return $(
      '.list_container__21Buw > table > tbody > tr:nth-child(1) > td:nth-child(6) > span > button'
    );
  }
  ...
  async deleteAcceptFunction() {
    await this.deleteBtn.click();
    await expect(this.deleteModalQuestion).toBeDisplayed();
    await expect(this.deleteModalQuestion).toBeHaveText(
      'Are you sure you want to delete this super admin?'
    );
